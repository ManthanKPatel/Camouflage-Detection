...
salDir = './Result/2020-CVPR-SINet-New/COD10K/'; %MoCA
Results_Save_Path = './Result/COD10K-try1';
gtDir = './Dataset/TestDataset/COD10K/GT/'; 
vv  '  '
Thresholds = 1:-1/255:0;
resPath = [salDir];  % video saliency map for evaluated models

filePath = [Results_Save_Path '/'];   % results save path

if ~exist(filePath, 'dir')
    mkdir(filePath);
end

fileID = fopen([filePath '_result.txt'], 'w')
...
for i = 1:fileNUM %skip the first and last gt file for some of the light-flow based method
    ...
    % fprintf('[Processing Info] Dataset: %s, SalSeq: %s (%d/%d), SalMapName: %s (%d/%d)\n',videofolder, seqfolder, seqnum, seqNUM, name, i, fileNUM);
...
fprintf(fileID, ' %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n',...
    seq_wFm(seqnum),seq_Sm(seqnum),seq_meanEm,seq_mae(seqnum),seq_meanDic,seq_meanIoU,seq_maxEm,seq_maxDic,seq_maxIoU,seq_meanSen,seq_maxSen,seq_meanSpe,seq_maxSpe);
fprintf(' seq_wFm  seq_Sm  seq_meanEm  seq_MAE seq_meanDic seq_meanIoU seq_maxEm seq_maxDice seq_maxIoU seq_meanSen seq_maxSen seq_meanSpe seq_maxSpe\n (%s) %.3f %.3f %.3f %.3f  %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f \n',...
    seq_wFm(seqnum),seq_Sm(seqnum),seq_meanEm,seq_mae(seqnum),seq_meanDic,seq_meanIoU,seq_maxEm,seq_maxDic,seq_maxIoU,seq_meanSen,seq_maxSen,seq_meanSpe,seq_maxSpe);  
% 
% %MAE
